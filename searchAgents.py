...
class PositionSearchProblem(search.SearchProblem):
    ...
    def isGoalState(self, state):
        isGoal = state == self.goal  # only the coordiantes
...
class CornersProblem(search.SearchProblem):
    ...
    def getSuccessors(self, state):
        ...
        for action in [Directions.NORTH, Directions.SOUTH, Directions.EAST, Directions.WEST]:
            ...
            if not self.walls[nextx][nexty]:
                # copy current corners, because lists can be weird
...
def cornersHeuristic(state, problem: CornersProblem):
    ...
    # Note: this heuristic is admissible and consitent, but does not satisfy the 1200 nodes expanded test (optional)
    # this might not be optimal, but we calculate the sum of the manhattan distances to all unvisited corners divided
    # by 2 this divison is added to be a true lower bound for approximation. It is non trivial by defintion.
    #
    # admissibility: this heuristic is admissible because the manhattan distance ignores any walls in its path
    #                and by dividing the sum of the MD to all unfound corners we always have a lower bound of
    #                the actual shortest path to visit all unfound corners
    #
    # consistency: the usual case is that there are 4 remaining corners. in this case the heuristic does not increase
    #              or decrease as the distance for 2 corners increases by at most one and the distance decreases
    #              for 2 corners by at most one; change in heuristic: (1+1-1-1) / 2 = 0
    #              extreme case of one remaining corner: here the manhattan distance decreases by at most 1 per step,
    #              which at a step cost of 1, is consistent
    distance = sum([util.manhattanDistance(state[0], cor)
                   for cor in unfound_corners]) / 2

    # return the sum of the manhattanDistance to all unvisited corners
    return distance
...
def foodHeuristic(state, problem):
    ...
    if useManhattan:
        ...
        distancesToAllFoods = [util.manhattanDistance(
            position, foodLocation) for foodLocation in foodList]
    else:
        ...
        # The heuristic is non-trivial by defintion.
        #
        # admissibility: this heuristic is admissible because we only return the distance to the food furthest away
        #                from the current position. Therefore, it is always less than the actual distance to collect
        #                all food items (except when only one food item remains, then it is the same distance)
        #
        # consistency: similar reasoning like the cornersHeuristic: the distance to the food furthest away can decrease
        #              by at most 1 which is consitent with a step cost of 1
        distancesToAllFoods = [mazeDistance(
            position, foodLocation, problem.startingGameState) for foodLocation in foodList]
    # return the max distance to any food or 0 if no food remains
...
class ClosestDotSearchAgent(SearchAgent):
    ...
    def findPathToClosestDot(self, gameState: GameState):
        ...
        "*** YOUR CODE HERE ***"
        assert not walls[startX][startY], 'starting point is a wall: ' + \
            str("({}, {})".format(startX, startY))
